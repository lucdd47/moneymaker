...
async def create_coin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_photo(
        photo=image_url,
        caption=f"🚀 *New Coin Launched!*\nName: *{name}*\nTicker: *{ticker}*\nMint: `{mint_address}`\n\n{description}\n\n🔗 [View on Pump.fun]({pumpfun_url})",
        parse_mode="Markdown"
    )
...
async def sell_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    market = fetch_market_data(mint_address)
    if market:
        await update.message.reply_text(f"📉 Post-Sell Market Cap: ${market['marketCap']}", parse_mode="Markdown")
    else:
